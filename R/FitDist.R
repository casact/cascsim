...
#' sindex <- new("Index", startDate = as.Date("2012-01-01"), monthlyIndex = 
#' cumprod(c(1,rep(1.03^(1/12),59))))
...
#' #xFit <- new("FitDist", observation=rawdata, trend=sindex,startDate = sdate,
#' #method="mme",ifreq=FALSE)
#' xFit <- setFitdata(xFit)
...
#' cumprod(c(1,rep(1.03^(1/12),59))))
#' sdate <- as.Date("2012-01-01")
#' rawdata <- as.data.frame(claimdata[(claimdata[,"LoB"]=="Auto" & claimdata[,"Type"]=="H" & 
#' claimdata[,"status"]=="Closed" & as.Date(claimdata[,"occurrenceDate"])>=sdate),][,c(9,7)])
#' xFit <- new("FitDist", observation=rawdata, trend=sindex,startDate = sdate,method="mle",ifreq=FALSE)
#' xFit <- setFitdata(xFit)
#' xFit@fitdata <- ifelse(xFit@fitdata <= 0, 1e-6, xFit@fitdata) #remove non positive number
...
#' xFit <- new("FitDist", observation=rawdata, trend=lindex,startDate = as.Date("2012-01-01"),
#' method="mle",ifreq=FALSE)
#' xFit <- setFitdata(xFit)
...
#' sindex <- new("Index", startDate = as.Date("2012-01-01"), monthlyIndex = 
#' cumprod(c(1,rep(1^(1/12),59))))
#' rawdata <- data.frame(rtlnorm(10000,3,2,5,100))
#' xFit <- new("FitDist", observation=rawdata, trend=sindex,startDate = as.Date("2012-01-01"),
#' method="mle",ifreq=FALSE)
#' xFit <- setFitdata(xFit)
#' xFit@fitdata <- ifelse(xFit@fitdata <= 0, 1e-6, xFit@fitdata) #remove non positive number
...
#' fitPlot(xFit)
#' xFit <- new("FitDist", observation=rawdata, trend=sindex,startDate = as.Date("2012-01-01"),
#' method="qme",probs=c(0.3,0.7),ifreq=FALSE)
#' xFit <- setFitdata(xFit)
#' xFit@fitdata <- ifelse(xFit@fitdata <= 0, 1e-6, xFit@fitdata) #remove non positive number
#' setTrialDist(xFit) <- new("Lognormal",p1=2,p2=1,min=5,max=100,truncated=TRUE)
#' xFit@soutput
